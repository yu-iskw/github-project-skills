...
Triage and project-board agents request explicit confirmation (or stop) before GitHub writes.

Wiki knowledge maintenance (`github-knowledge-maintainer`) may proceed with `{owner}/{repo}` from
`gh repo view` without a project config. Still STOP when a config exists and `owner.login` / `name`
from `gh repo view` disagree with it. Never treat the authenticated username as `{owner}`.

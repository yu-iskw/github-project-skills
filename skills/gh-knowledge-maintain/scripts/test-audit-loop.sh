...
has_wiki="$(echo "${payload}" | jq -r .has_wiki)"
wiki_remote="$(echo "${payload}" | jq -r .wiki_remote)"
...
echo "== 4. parse + validate Architecture candidate =="
node skills/gh-wiki-diagrams/scripts/mermaid_parse.mjs \

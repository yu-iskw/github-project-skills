...
1. Resolve `{owner}/{repo}` with `bash skills/gh-knowledge-maintain/scripts/repo-identity.sh`.
2. Collect evidence with `gh-knowledge-maintain` (`scripts/collect-delta.sh`). First run omits `--checkpoint`.

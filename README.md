...
This installs all 9 agent skills, 3 subagents, and the `/knowledge-maintain` and `/knowledge-audit` slash commands into Claude Code automatically.
...
- `make test`: Plugin manifest (skill directories, slash commands), helper scripts, Mermaid fixtures, local Wiki git loop, live `gh` evidence, and the knowledge audit loop.

...
2. **Discovery**: Use `gh-issue-management` to search for high-priority bugs and requested features (e.g., `gh search issues --label bug --state open`).
3. **Milestone Assignment**: Use `gh-issue-management` to group identified issues into a specific release milestone (`gh issue edit <number> --milestone <name>`).
4. **Project Organization**:
   - Use `gh-project-management` to list available custom fields (e.g., "Sprint", "Target Version") via `gh project field-list`.
   - Use `gh-project-management` to batch-update project items with the new planning data via `gh project item-edit`.
5. **Tracking**: Use `gh-issue-management` to list open issues filtered by milestone to monitor progress toward the release goal.
...
0. **Prerequisite**: Run `gh-set-active-project` once to create `.github/project-config.json`.
1. **Context**: `gh-verifying-context` auto-verifies against the config silently.
2. **Audit**: Use `gh-issue-management` to search for issues that belong in a different repository (`gh search issues --repo <owner>/<repo>`).
3. **Transfer**: Use `gh-issue-management` to move each issue to the correct destination repository (`gh issue transfer <number> <destination-repo>`).
4. **Linkage**: Use `gh-issue-management` to comment on both the original and transferred issue to preserve traceability (`gh issue comment <number> --body "..."`).
5. **Cleanup**: Use `gh-issue-management` to apply labels on the transferred issue in the destination repository to match its triage standards.
